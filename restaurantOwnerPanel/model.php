 ...
 * mbe.ro
 *
 * @author     raj kaushal <http://phpexpertgroup.com/> <info@phpexpertgroup.com>
...
class mysql {
	...
	function __construct($db=array()) {
		$default = array(
			'host' => 'localhost',
			'user' => 'phpexubr_megafly',
			'pass' => 'raj786',
			'db' => 'phpexubr_flymeallmega'
		);
	...
	function update($table=null,$array_of_values=array(),$conditions='FALSE') {
		...
		foreach ($array_of_values as $field => $value) {
			...
			else $what_to_set []= "`$field`=N'".mysql_real_escape_string($value,$this->con)."'";
		}
	...
	function insert($table=null,$array_of_values=array()) {
		...
		foreach ($array_of_values as $id => $value) {
			...
			else $values[]="N'".mysql_real_escape_string($value,$this->con)."'";
		}
