...
$qry=mysql_query("SELECT * FROM resto_order where restoid='".$_GET['restaurant_id']."' and odr_date='$curdate' order by order_id desc");
}
else
{		
...
if($Commission!='')
 ...
 $Cpl='0.00';
            	...
            	$dSmOdr+=($data['ordPrice']*$CuisineData['restaurant_commission'])/100;
					...
					<td>'.$AdminDataLoginVal['website_CurrencySymbole'].' '.$data['ordPrice'].'</td>
                    <td>'.$CuisineData['restaurant_commission'].' % = '.$AdminDataLoginVal['website_CurrencySymbole'].' '.$Cpl
	...
	<td  colspan="3" style="border:none;" align="center"><strong style="color:#000080; font-size:14px;">Total : '.$AdminDataLoginVal['website_CurrencySymbole'].''.$totalSum.'</strong></td>
		<td colspan="3" style="border:none;" align="right"><strong style="color:#000080; font-size:14px;">Commission : '.$AdminDataLoginVal['website_CurrencySymbole'].' '.$dSmOdr.'</strong></td>
...
$pdf->Output('TodayRestaurantOrder.pdf', 'I');
