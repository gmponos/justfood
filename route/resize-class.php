...
class _image {
    ...
    public $padColour         = '';    // set background padding colour - '#XXXXXX' or 'transparent' (transparent requires PNG type)
    ...
    //    if(!file_exists($path)) { mkdir($path, 0777); }
        ...
        if(file_exists($path.$preFix.$imgName)) {
            switch($this->duplicates) {
                ...
                default: // make unique
                    ...
                    $imgName = $im.rand(0,1000000).'.'.$this->imgType;
            }
        }
        ...
        switch(strtolower($this->returnType)) {
            case 'array': 
                ...
                return $_ar; break;
            case 'fullpath':
                return (file_exists($path.$preFix.$imgName)) ? $preFix.$imgName : false; break;
            case 'blob':
                if(file_exists($path.$preFix.$imgName)) {
                    $fo = fopen($path.$preFix.$imgName, 'r');
                    $blob = mysql_real_escape_string(fread($fo, filesize($path.$preFix.$imgName)));
