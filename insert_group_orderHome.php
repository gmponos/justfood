...
if(isset($_POST['RestaurantGroupOrderEmail1'])){
$resdata=mysql_fetch_assoc($prix->showtabledata('tbl_restaurantAdd','id',$_POST['restaurant_id']));
$ProductServiceInter=array("restaurant_id"=>$_POST['restaurant_id'],"restaurant_name"=>mysql_real_escape_string($resdata['restaurant_name']),"RestaurantGroupOrderName"=>mysql_real_escape_string($_POST['RestaurantGroupOrderName1']),"RestaurantGroupOrderEmail"=>$_POST['RestaurantGroupOrderEmail1'],"RestaurantGroupOrderMessage"=>mysql_real_escape_string($_POST['RestaurantGroupOrderMessage1']),"ip"=>$_SERVER['REMOTE_ADDR'],"sessoionId_groupOrder"=>session_id(),"created_date"=>date('d l Y'));
$CuisineInsert=$dbObj->dataInsert($ProductServiceInter,"tbl_restaurantGroupOrder");
if($CuisineInsert)
{
$subject ="Let's Order from ".$resdata['restaurant_name'];
$to=$_POST['RestaurantGroupOrderEmail1'].','.$RegistrationDataLoginVal['contactusemail'];
              ...
              <td rowspan='2'> 
                ...
                <table cellpadding='0' cellspacing='0' width='730'>
                  <tbody>
                   ...
                   <tr>
                          <td > 
<h2 style='color:#000066;margin-left:10px; margin-top:10px;'>Hi ".ucwords($_POST['RestaurantGroupOrderName1'])." !</h2>
<p style='padding:10px 0 0 20px;margin-left:20px;margin-bottom:1em;font-family:Arial,Helvetica,sans-serif;font-size:17px;color:#3a3a3a'>
                          You have initiated a group order from ".$resdata['restaurant_name']."</p>
                       
<div style='padding:10px; color:#CA0000; font-size:18px; margin-left:50px;'> Go ahead and forward this  <a href='".$DomainName."restaurant.php?restaurants=".$_POST['restaurant_id']."-".urldecode($resdata['restaurant_name']).".html'>link</a> to others to place the order with you.</div>
	...
	{
	unset($_POST['RestaurantGroupOrderEmail1']);
	header("location:restaurant.php?restaurants=".$_POST['restaurant_id']."-".urldecode($resdata['restaurant_name']).".html");
	}
}
}
?>