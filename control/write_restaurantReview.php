										...
										<div class="row-fluid" >
											<div  class=" span12">
												<form id="SignupForm" action="" method="post">
                                                    ...
                                                    <tr>
    <td><label>Restaurant  <span class="f_req">*</span></label></td>
    <td><select style="width:317px;" name="RestaurantReviewId"  required  id="RestaurantReviewId">
    <option value="" selected="selected">Select</option>
    <?php $QyeryDat=mysql_query("select * from tbl_restaurantAdd where status='0' order by restaurant_name asc");
	while($offerTypeData=mysql_fetch_assoc($QyeryDat)){
	 ?>
											  <option value="<?php echo $offerTypeData['id'];?>" <?php if($CuisineData['RestaurantReviewId']==$offerTypeData['id']){?> selected="selected" <?php } ?>><?php echo $offerTypeData['restaurant_name']; ?></option>
											<?php } ?>
											</select></td>
  </tr>
