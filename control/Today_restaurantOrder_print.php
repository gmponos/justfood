...
$excel=new ExcelWriter("TodayRestaurantOrder.xls");
if($excel==false)	
$excel->error;
$myArr=array("");
$myArr=array("Today Restaurant Order Report");
...
$qry=mysql_query("SELECT * FROM resto_order where odr_date='$curdate' and restoid='".$_GET['restaurant_id']."'  order by order_id desc");
}
else
{
$qry=mysql_query("SELECT * FROM resto_order where odr_date='$curdate'  order by order_id desc");
}
if($qry!=false)
{
	$i=1;
				...
				<div class="container-fluid">
					<div class="tabbable main-tabs">
						<ul class="nav nav-tabs">
							<li class="active" style="background-color:#f2f2f2;"><a href="#mainFormElements" style="background-color:#f2f2f2;" data-toggle="tab"><i class="icon-file"></i>Today Restaurant Orders Report</a></li>
						</ul>
									...
									<div class="well">
										<div class="navbar">
											<div class="navbar-inner">
												<a class="brand" href="#">Today Restaurant Orders Report </a>
											</div>
										</div>
										<div class="row-fluid">
    <div class="span12">
...
{
window.location.href="Today_restaurantOrder_print.php?sort="+str+"&f="+Dvalue;
}
</script>

    <table width="70%" border="0">
  <tr>
    <td><a href="TodayRestaurantOrder.xls" class="btn btn-info" title="Export into CVS Format" >Export into Excel Format</a></td>
    <td>
    <?php if($_GET['restaurant_id']!=''){ ?>
    <a href="TodayexportOrderCVS.php?restaurant_id=<?php echo $_GET['restaurant_id'];?>" class="btn btn-primary" title="Export into CVS Format" >Export into CVS Format</a>
    <?php } else { ?>
     <a href="TodayexportOrderCVS.php" class="btn btn-primary" title="Export into CVS Format" >Export into CVS Format</a>
    <?php } ?>
    </td>
    <td>
    <?php if($_GET['restaurant_id']!=''){ ?>
    <a href="tcpdf/examples/Today_restaurantOrder_PDF.php?restaurant_id=<?php echo $_GET['restaurant_id'];?>" target="_blank" class="btn btn-tigadua" title="Generate PDF" >Generate PDF</a>
    <?php } else { ?>
        <a href="tcpdf/examples/Today_restaurantOrder_PDF.php" target="_blank" class="btn btn-tigadua" title="Generate PDF" >Generate PDF</a>
    <?php } ?>
    </td>
    <td>
    <?php if($_GET['restaurant_id']!=''){ ?>
    <a href="today_all_order_print.php?restaurant_id=<?php echo $_GET['restaurant_id'];?>" class="btn btn-duasembilan" title=" Print Restaurant Orders" target="_blank" > Print Restaurant Orders</a>
    <?php } else { ?>
        <a href="today_all_order_print.php" class="btn btn-duasembilan" title=" Print Restaurant Orders" target="_blank" > Print Restaurant Orders</a>
...
xmlhttp.send();
                                             ...
                                             <br /><br />
                                             <form action="Today_restaurantOrder_print.php" method="get" name="userform" id="userform">
<table width="100%" border="0" style="padding:15px;">
 ...
 $curdate=date("Y-m-d");
 if($_GET['restaurant_id']!=''){
$sql = "SELECT * FROM resto_order  where odr_date='$curdate' and restoid='".$_GET['restaurant_id']."' $sortBy ";
}
else
{
$sql = "SELECT * FROM resto_order  where odr_date='$curdate' $sortBy ";
}
 ...
 $OwnerData=mysql_fetch_assoc($OwnerQuery);
		 ?>
				<tr>
                    ...
                    <td><?php echo $CuisineData['restaurant_commission']; ?> % = <?php echo $AdminDataLoginVal['website_CurrencySymbole'];?> <?php 
					 $Commission= ($data['ordPrice']*$CuisineData['restaurant_commission'])/100; ?>
                    <?php if($Commission!=''){
