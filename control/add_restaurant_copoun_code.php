				...
				<div class="container-fluid">
					<div class="tabbable main-tabs">
						<ul class="nav nav-tabs">
							<li class="active" ><a href="#mainFormElements"  data-toggle="tab"><i class="icon-file"></i><?php  if($_GET['eid']=='')
										   { ?>
...
return true;
  ...
  <tr>
										...
										<div class="row-fluid" >
										   ...
										   ?>
												...
												<form id="SignupForm" name="SignupForm" action="<?php echo $url; ?>" method="post" onsubmit="return CouponCodeValidate();" enctype="multipart/form-data">
                                                    ...
                                                    <tr>
    <td><label for="RestauranCouponId">Restaurant Name <span style="color:#FF0000;">*</span></label></td>
    <td>	<select class="chzn-select" data-placeholder="Select Restaurant Name..." required name="RestauranCouponId" id="RestauranCouponId" onMouseOver="return clearFieldValue(this);" onClick="return clearFieldValue(this);" style="width:317px;">
										<option value="0">Select Restaurant</option>
											  ...
											  {
											   $restaurant_id=$CuisineData['RestauranCouponId'];
											  }
    ...
    <td><label for="RestauranCouponName">Coupon Title <span style="color:#FF0000;">*</span></label></td>
  ...
  <tr>
    <td><label for="restaurant_country">Coupon Type <span style="color:#FF0000;">*</span></label></td>
    <td><table> <tr>
    <td><input type="radio" name="RestauranCouponType" <?php if($_GET['eid']==''){?>checked="checked" <?php } ?>   class="ofrty" id="offrP" value="p" <?php if($CuisineData['RestauranCouponType']=='p'){ ?> checked="checked" <?php } ?> onClick="toggleTables1(this.value);"/> Price</td>
    <td><input type="radio" name="RestauranCouponType" class="ofrty" id="offrP" <?php if($CuisineData['RestauranCouponType']=='pr'){ ?> checked="checked" <?php } ?> value="pr" onClick="toggleTables1(this.value);"/> Percentage</td>
  </tr></table></td>
  </tr>
  <tr>
    <td><label for="RestauranCouponNo">Coupon Code <span style="color:#FF0000;">*</span></label></td>
    <td><input name="randomfield" id="randomfield" required onMouseOver="return clearFieldValue(this);" onClick="return clearFieldValue(this);" placeholder="New Coupon Code" type="text" value="<?php echo $CuisineData['RestauranCouponNo']; ?>"   style="width:200px;"/> &nbsp; <input type="button" value="Generate Coupon Code" onClick="randomString();" class="btn btn-dualima" style="cursor:pointer;"></td>
    <td><label for="RestauranCouponPrice">Coupon Price <span style="color:#FF0000;">*</span>  <span style="display:none;" id="showsymoble">(%)</span></label></td>
    <td><input name="RestauranCouponPrice" onkeypress='return isNumberKey(event);' id="RestauranCouponPrice" required onMouseOver="return clearFieldValue(this);" onClick="return clearFieldValue(this);" placeholder="New Coupn Amount" value="<?php echo $CuisineData['RestauranCouponPrice']; ?>" type="text"  style="width:300px;" /></td>
  </tr>
...
<option value="1" <?php if($_GET['statusid']==1){ ?> selected="selected" <?php } ?> >Deactivated</option>
        ...
        ?>
                                                ...
                                                <td><?php echo ucwords($row['RestauranCouponPrice']);?>
                                                <?php if($row['RestauranCouponType']=='pr'){ ?>%<?php } else { ?><?php echo $AdminDataLoginVal['website_CurrencySymbole'];?> <?php } ?>
                                                </td>
                        ...
                        <?php if($row['status']==0){ ?>
                        <a href="InsertPhp.php?tag=ResCouponCodeActivate&active=1&statusid=<?php echo $row['id'];?>" class="btn btn-duasembilan" onClick="return confirm('Are you sure to do this action.');" title="Activate/Deactivate Coupon Code">Deactivate</a><?php } else {?>
