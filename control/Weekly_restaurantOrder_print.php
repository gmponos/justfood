...
$excel=new ExcelWriter("WeeklyRestaurantOrder.xls");
if($excel==false)	
$excel->error;
$myArr=array("");
$myArr=array("Weekly Restaurant Order Report");
				...
				<div class="container-fluid">
					<div class="tabbable main-tabs">
						<ul class="nav nav-tabs">
							<li class="active" style="background-color:#f2f2f2;"><a href="#mainFormElements" style="background-color:#f2f2f2;" data-toggle="tab"><i class="icon-file"></i>Weekly Restaurant Orders Report</a></li>
						</ul>
									...
									<div class="well">
										<div class="navbar">
											<div class="navbar-inner">
												<a class="brand" href="#">Weekly Restaurant Orders Report </a>
											</div>
										</div>
										<div class="row-fluid">
    <div class="span12">
...
{
window.location.href="Weekly_restaurantOrder_print.php?sort="+str+"&f="+Dvalue;
}
</script>
    <table width="70%" border="0">
  <tr>
    <td><a href="WeeklyRestaurantOrder.xls" class="btn btn-info" title="Export into CVS Format" >Export into Excel Format</a></td>
    <td>
    <?php if($_GET['restaurant_id']!=''){ ?>
    <a href="WeeklyexportOrderCVS.php?restaurant_id=<?php echo $_GET['restaurant_id'];?>" class="btn btn-primary" title="Export into CVS Format" >Export into CVS Format</a>
    <?php } else { ?>
     <a href="WeeklyexportOrderCVS.php" class="btn btn-primary" title="Export into CVS Format" >Export into CVS Format</a>
    <?php } ?>
    </td>
    <td>
     <?php if($_GET['restaurant_id']!=''){ ?>
    <a href="tcpdf/examples/Weekly_restaurantOrder_PDF.php?restaurant_id=<?php echo $_GET['restaurant_id'];?>" target="_blank" class="btn btn-tigadua" title="Generate PDF" >Generate PDF</a>
    <?php } else{ ?>
    <a href="tcpdf/examples/Weekly_restaurantOrder_PDF.php" target="_blank" class="btn btn-tigadua" title="Generate PDF" >Generate PDF</a>
    <?php } ?>
    </td>
    <td>
     <?php if($_GET['restaurant_id']!=''){ ?>
    <a href="weekly_all_order_print.php?restaurant_id=<?php echo $_GET['restaurant_id'];?>" class="btn btn-duasembilan" title=" Print Restaurant Orders" target="_blank" > Print Restaurant Orders</a>
    <?php } else { ?>
      <a href="weekly_all_order_print.php" class="btn btn-duasembilan" title=" Print Restaurant Orders" target="_blank" > Print Restaurant Orders</a>
    <?php } ?>
    </td>
  </tr>
...
xmlhttp.send();
                                             ...
                                             <br /><br />
                                             <form action="Weekly_restaurantOrder_print.php" method="get" name="userform" id="userform">
...
$CuisineQuery = $dbb->showtabledata("tbl_restaurantAdd","id",$data['restoid']);
 ...
 $Cpl='0.00';
		 ...
		 ?>
                ...
                <tr>
                <td colspan="8"><strong style="color:#FF0000; padding:10px; font-size:16px;">This Week ! No Orders available for Printable</strong></td>
                </tr>
                <?php } ?>
                <tr>
