...
if(isset($_POST['deleteall'])) {
	...
	for($i=0; $i <$id_count; $i++) {
		...
		if(!$query) { 
		header("location:manage_commercialBanner.php");
		}
	}
...
<div id="page-wrap">
		...
		if($_GET['ResBannerNameID']!='' && $_GET['statusid']!='' )
		{
		
		$url="manage_commercialBanner.php?ResBannerNameID=".$_GET['ResBannerNameID']."&statusid=".$_GET['statusid']."&";
		$where="ResBannerNameID='".$_GET['ResBannerNameID']."' and status='".$_GET['statusid']."' and bannerPage='1'";
		
		$qur="SELECT * FROM {$statement} where ResBannerNameID='".$_GET['ResBannerNameID']."' and status='".$_GET['statusid']."' and bannerPage='1' LIMIT {$startpoint} , {$limit}";
		}
		elseif($_GET['ResBannerNameID']!='')
		{
		$url="manage_commercialBanner.php?ResBannerNameID=".$_GET['ResBannerNameID']."&";
		$where="ResBannerNameID='".$_GET['ResBannerNameID']."' and bannerPage='1'";
		
		$qur="SELECT * FROM {$statement} where ResBannerNameID='".$_GET['ResBannerNameID']."' LIMIT {$startpoint} , {$limit}";
		}
		
		elseif($_GET['statusid']!='')
		{
		$url="manage_commercialBanner.php?statusid=".$_GET['statusid']."&";
		$where="status='".$_GET['statusid']."' and bannerPage='1'";
		
		$qur="SELECT * FROM {$statement} where status='".$_GET['statusid']."' and bannerPage='1' LIMIT {$startpoint} , {$limit}";
		}		
		else
		{
		$url="manage_commercialBanner.php?bannerPage=1&";
		$where=" bannerPage='1'";
		$qur="SELECT * FROM {$statement} where bannerPage='1' LIMIT {$startpoint} , {$limit}";
		}
		//echo $qur;
    ...
    <form action="manage_commercialBanner.php" method="get" id="userform" name="userform">
      <table width="100%" border="0">
  <tr>
  
  <td><label for="Name">Restaurant Name</label></td>
    <td>
    <select class="chzn-select" data-placeholder="Select Restaurant Name..." id="ResBannerNameID" name="ResBannerNameID" onChange="document.userform.submit();" style="width:350px;" >
											  <?php 
											  $StateQuery = $dbb->showtable("tbl_restaurantAdd","restaurant_name","asc");
                                              while($StateData=mysql_fetch_assoc($StateQuery)){
											  ?>
                                              <option value="<?php echo $StateData['id']; ?>" <?php if($_GET['ResBannerNameID']==$StateData['id']){ ?> selected <?php } ?>><?php echo ucwords($StateData['restaurant_name']); ?></option>
                                              <?php } ?>
											
											</select>
    </td>
      <td><strong style="font-size:14px; font-weight:bold;">Status : </strong> </td>
    <td> <select  class="chzn_a span8"  name="statusid" style="width:250px;" id="statusid" onChange="document.userform.submit();" >
...
<option value="1" <?php if($_GET['statusid']==1){ ?> selected="selected" <?php } ?> >Deactivated</option>
        ...
        ?>
                                                ...
                                                <td><input type="checkbox" value="<?php echo $row['id']; ?>" name="data[]" id="data"></td>
                                                    ...
                                                    <td><?php 
													 $StQuery = mysql_fetch_assoc($dbb->showtabledata("tbl_restaurantAdd","id",$row['ResBannerNameID']));
													echo ucwords($StQuery['restaurant_name']);?></td>
                                                     <td>
                                                     <?php
													  if($row['bannerType']=="BannerImg")
													  {?>
													 <img src="BannerImage/small/<?php echo $row['BannerImg'];?>">
													 <?php }
													  if($row['bannerType']=="Bannercode")
													  {
													echo html_entity_decode($row['BannerCode']);
													  }
													   ?>
                                                     </td>
                                                      <td><?php
                      ...
                      <a href="add_restaurantBanner.php?eid=<?php echo $row['id'];?>" class="btn btn-dualima" title="Edit Restaurant Commercial Banner" >Edit</a>
