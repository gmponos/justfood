...
$excel=new ExcelWriter("allRestaurantOrder.xls");
if($excel==false)	
$excel->error;
$myArr=array("");
$myArr=array("All Restaurant Order Report");
				...
				<div class="container-fluid">
					<div class="tabbable main-tabs">
						<ul class="nav nav-tabs">
							<li class="active" style="background-color:#f2f2f2;"><a href="#mainFormElements" style="background-color:#f2f2f2;" data-toggle="tab"><i class="icon-file"></i>All Restaurant Orders Report</a></li>
						</ul>
									...
									<div class="well">
										<div class="navbar">
											<div class="navbar-inner">
												<a class="brand" href="#">All Restaurant Orders Report </a>
											</div>
										</div>
										<div class="row-fluid">
    <div class="span12">
...
{
window.location.href="all_restaurant_order_print.php?sort="+str+"&f="+Dvalue;
}
</script>

    <table width="70%" border="0">
  <tr>
    <td><a href="allRestaurantOrder.xls" class="btn btn-info" title="Export into CVS Format" >Export into Excel Format</a></td>
    <td>
    <?php if($_GET['restaurant_id']!=''){ ?>
    <a href="allexportOrderCVS.php?restaurant_id=<?php echo $_GET['restaurant_id'];?>" class="btn btn-primary" title="Export into CVS Format" >Export into CVS Format</a>
    <?php } else { ?>
     <a href="allexportOrderCVS.php" class="btn btn-primary" title="Export into CVS Format" >Export into CVS Format</a>
    <?php } ?>
    </td>
    <td>
    <?php if($_GET['restaurant_id']!=''){ ?>
    <a href="tcpdf/examples/allRestaurantOrder.php?restaurant_id=<?php echo $_GET['restaurant_id'];?>" target="_blank" class="btn btn-tigadua" title="Generate PDF" >Generate PDF</a>
    <?php } else { ?>
        <a href="tcpdf/examples/allRestaurantOrder.php" target="_blank" class="btn btn-tigadua" title="Generate PDF" >Generate PDF</a>
    <?php } ?>
    </td>
    <td>
    <?php if($_GET['restaurant_id']!=''){ ?>
    <a href="all_restaurant_order_print_table.php?restaurant_id=<?php echo $_GET['restaurant_id'];?>" class="btn btn-duasembilan" title=" Print Restaurant Orders" target="_blank" > Print Restaurant Orders</a>
    <?php } else { ?>
        <a href="all_restaurant_order_print_table.php" class="btn btn-duasembilan" title=" Print Restaurant Orders" target="_blank" > Print Restaurant Orders</a>
...
xmlhttp.send();
                                             ...
                                             <br /><br />
                                             <form action="all_restaurant_order_print.php" method="get" name="userform" id="userform">
<table width="100%" border="0" style="padding:15px;">
 ...
 $curdate=date("Y-m-d");
 if($_GET['restaurant_id']!=''){
$sql = "SELECT * FROM resto_order  where restoid='".$_GET['restaurant_id']."' $sortBy ";
}
else
{
$sql = "SELECT * FROM resto_order   $sortBy ";
}
 ...
 $OwnerData=mysql_fetch_assoc($OwnerQuery);
		 ?>
				<tr>
                    ...
                    <td><?php echo $CuisineData['restaurant_commission']; ?> % = <?php echo $AdminDataLoginVal['website_CurrencySymbole'];?> <?php  $Commission= ($data['ordPrice']*$CuisineData['restaurant_commission'])/100; ?>
                    <?php if($Commission!=''){
