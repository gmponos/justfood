...
function toggleTables1(which)
        {
    if(which == "pr" ) {
        document.getElementById('showsymoble').style.display='block';
       }
	  
        else
		{
		document.getElementById('showsymoble').style.display = "none";
		}
}
...
xmlhttp.open("post","ResFatchOffer.php?q="+str,true);
xmlhttp.send();
}
    ...
    <td>	<select  data-placeholder="Select Restaurant..." required onMouseOver="return clearFieldValue(this);" onClick="return clearFieldValue(this);"  id="restaurant_id" name="restaurant_id" style="width:317px;" >
    ...
    <option value="">Select</option>
      <option value="1" <?php if($CuisineData['OrderDiscountType']==1){ ?> selected <?php } ?>>Always</option>
        <option value="2" <?php if($CuisineData['OrderDiscountType']==2){ ?> selected <?php } ?>>First Order</option>
                                             ...
                                             <tr>
    <td colspan=""><label for="restaurant_website">Order Discount show <span style="color:#FF0000;">*</span></label></td>
    <td colspan="3">
  ...
  <tr ><td colspan="3"><hr></td></tr>
   ...
   <tr >
    ...
    <td colspan="3">
  <select class="chzn-select" data-placeholder="Select Restaurant..." multiple="multiple"  id="OrderDiscountmenuCategory" name="OrderDiscountmenuCategory[]" style="width:317px;" >
    <option value="">Select</option>
       <?php $StateQuery = mysql_query("select * from tbl_restMenuCategory group by restaurantMenuName order by restaurantMenuName asc  ");
								 while($StateData=mysql_fetch_assoc($StateQuery)){
											  ?>
                                              <option value="<?php echo $StateData['id']; ?>" <?php if(strstr($CuisineData['OrderDiscountmenuCategory'],$StateData['id'])){?> selected="selected" <?php } ?> ><?php echo ucwords($StateData['restaurantMenuName']); ?></option>
                                              <?php } ?>
          </select>
    </td>
                                            ...
                                            <tr>
    <td colspan=""><label for="restaurant_website">Start Order Price <span style="color:#FF0000;">*</span></label></td>
    <td colspan="3">
    <input type="text" name="RestaurantOfferStartPrice" onMouseOver="return clearFieldValue(this);" onClick="return clearFieldValue(this);"  id="RestaurantOfferStartPrice" style="width:300px;" onkeypress='return isNumberKey(event);' required value="<?php echo $CuisineData['RestaurantOfferStartPrice']; ?>"  />
    </td>
   
  </tr> 
                                            <tr>
    <td><label for="restaurant_country">Discount Type <span style="color:#FF0000;">*</span></label></td>
    <td><table width="40%"> <tr>
    <td><input type="radio" name="RestaurantoffrType" required <?php if($_GET['eid']==''){?>checked="checked" <?php } ?>  class="ofrty" id="offrP" value="p" <?php if($CuisineData['RestaurantoffrType']=='p'){ ?> checked="checked" <?php } ?> onClick="toggleTables1(this.value);"/> Price Discount</td>
    <td><input type="radio" name="RestaurantoffrType" required class="ofrty" id="offrP" <?php if($CuisineData['RestaurantoffrType']=='pr'){ ?> checked="checked" <?php } ?> value="pr" onClick="toggleTables1(this.value);"/> Percentage Discount
   ...
   <tr>
    <td colspan=""><label for="restaurant_website">Discount Price <span style="color:#FF0000;">*</span> <span style="display:none;" id="showsymoble">(%)</span></label></td>
    <td colspan="3">
    <input type="text" name="RestaurantOfferPrice" required onMouseOver="return clearFieldValue(this);" onkeypress='return isNumberKey(event);' style="width:300px;" onClick="return clearFieldValue(this);"  id="RestaurantOfferPrice" value="<?php echo $CuisineData['RestaurantOfferPrice']; ?>"  />
    </td>
   
  </tr>
  <tr>
    <td><label for="RestaurantOfferStartDate">Discount Start Date <span style="color:#FF0000;">*</span></label></td>
    ...
    <tr>
    <td><label for="RestaurantOfferEndDate">Discount End Date <span style="color:#FF0000;">*</span></label></td>
    <td><input value="<?php echo $CuisineData['RestaurantOfferEndDate']; ?>" required data-date-format="mm/dd/yyyy" type="text" name="RestaurantOfferEndDate" id="RestaurantOfferEndDate"  class="datePicker"   style="width:300px;" onblur="return RestaurantDateCheck();" onmouseout="return RestaurantDateCheck();"  /></td>
  </tr>
 
    <tr>
    <td colspan=""><label for="restaurant_website">Discount Description</label></td>
    <td colspan="3">
    <textarea name="RestaurantOfferDescription" required onMouseOver="return RestaurantDateCheck();" onClick="return clearFieldValue(this);"   id="RestaurantOfferDescription" class="twys" style="width:700px; height:80px;"><?php echo $CuisineData['RestaurantOfferDescription']; ?></textarea>
    </td>
