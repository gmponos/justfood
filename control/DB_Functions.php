...
public function storeUpdateEmailSetting($supportemail,$registrationemail,$contactusemail,$orderemail,$invoiceemail,$bookingemail,$id) {
       $result = mysql_query("Update tbl_EmailSetting set supportemail='$supportemail',registrationemail='$registrationemail',contactusemail='$contactusemail',orderemail='$orderemail',bookingemail='$bookingemail' where id='$id'");
        if ($result) {
           return $result;
        } else {
            return false;
...
public function storeAddResUser($fname,$lname,$countryID,$stateID,$cityName,$zipcode,$gender,$landmark,$user_phone,$address,$user_email,$user_pass) {
        $sql = mysql_query("SELECT * from tbl_user WHERE  user_email ='$user_email' ");
        $no_rows = mysql_num_rows($sql);
		if ($no_rows == 0) 
		{
		$result = mysql_query("INSERT INTO tbl_user(fname,lname,countryID,state_name,city_name,zipcode,gender,landmark,user_phone,address,user_email,user_pass,status,ip,input_date) VALUES(N'$fname',N'$lname','$countryID','$stateID',N'$cityName','$zipcode','$gender',N'$landmark','$user_phone',N'$address','$user_email','$user_pass','0','".$_SERVER['REMOTE_ADDR']."', NOW())");
        // check for successful store
        if ($result) {
           return $result;
        } else {
	 ...
	 public function storeUpdateResUser($fname,$lname,$countryID,$stateID,$cityName,$zipcode,$gender,$landmark,$user_phone,$address,$user_email,$user_pass,$id) {
       $result = mysql_query("Update tbl_user set fname=N'$fname',lname=N'$lname',user_email='$user_email',user_pass='$user_pass',countryID='$countryID',state_name='$stateID',city_name=N'$cityName',zipcode='$zipcode',gender='$gender' ,landmark='$landmark'  ,user_phone='$user_phone'  ,address=N'$address' where id='$id'");
        if ($result) {
           return $result;
        } else {
            return false;
